    ...
    <aside>
        <div class="container">
            <div class="wrapper" :class="{deviceMenuVisible: devicesMenu}">
                <menu>
                    <li v-if="!$store.state.preferences.showConstellation"><button @click="showConstellation(); $ga.event('player', 'toggle-constellation')" :class="{disabled: !$store.getters['tracks/playlistLoaded'] || $store.state.loading.playlistGenerating}">
                    ...
                    <li v-if="$store.state.preferences.showConstellation"><button @click="showPlaylist(); $ga.event('player', 'toggle-playlist')" class="fillMagenta" :class="{disabled: !$store.getters['tracks/playlistLoaded'] || $store.state.loading.playlistGenerating}">
                    ...
                    <li v-if="playerReadyAndConnected"><button @click="play(); $ga.event('player', 'play', 'play')" v-if="$store.getters['tracks/stopped']" :class="{disabled: !$store.getters['tracks/playlistLoaded']}">
                    ...
                    <li v-if="playerReadyAndConnected"><button @click="resume(); $ga.event('player', 'play', 'resume')" v-if="$store.getters['tracks/paused']" :class="{disabled: !$store.getters['tracks/playlistLoaded']}">
                        <svg xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" viewBox="0 0 32 32" x="0px" y="0px" aria-labelledby="uniqueTitleID" role="img"><title id="uniqueTitleID">Play Track</title><path d="M3,0.25V31.71L30.25,16ZM5,3.71L26.25,16,5,28.24V3.71Z"></path></svg>
                    </button></li>
                    <li v-if="playerReadyAndConnected"><button @click="pause(); $ga.event('player', 'pause')" v-if="$store.getters['tracks/playing']" :class="{disabled: !$store.getters['tracks/playlistLoaded']}">
                    ...
                    <li v-if="playerReadyAndConnected"><button @click="previous(); $ga.event('player', 'previous')" :class="{disabled: !canSeek || !$store.getters['tracks/canSkipBackward']}">
                    ...
                    <li v-if="playerReadyAndConnected"><button @click="next(); $ga.event('player', 'next')" :class="{disabled: !canSeek || !$store.getters['tracks/canSkipForward']}">
                    ...
                    <li v-if="premiumUser" class="devicePicker">
                        <button @click="toggleDevicePicker(); $ga.event('player', 'toggle-device-picker')">
                    ...
                    <li v-if="!premiumUser" class="devicePicker savePlaylist">
                        <button @click="createPlaylistAndPlay(); $ga.event('player', 'play', 'create-playlist')" :disabled="savingPlaylist || $store.state.loading.tracksDownloading || $store.state.loading.playlistGenerating" :class="{disabled: savingPlaylist || $store.state.loading.tracksDownloading || $store.state.loading.playlistGenerating}">
        ...
        <div v-if="devicesMenu" class="devicesMenu">
            <ol>
                <li v-for="device in devices" :class="{active: device.active}" @click="transferDevice(device.id); $ga.event('player', 'select-device', device.type)" v-if="device.type != 'Phosphorescence' || device.isPrimary">
                ...
                <li @click="createPlaylistAndFollow(); $ga.event('player', 'create-playlist')">
                ...
                <li @click="refreshDevices(); $ga.event('player', 'refresh-devices')">
                    <button class="deviceButton">
...
<style scoped>
    ...
    menu button.fillMagenta:not(.disabled) svg:hover {
        fill: magenta;
        stroke: magenta;
    }
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            showConstellation() {
                this.$store.commit('preferences/showConstellation');
            },
            showPlaylist() {
