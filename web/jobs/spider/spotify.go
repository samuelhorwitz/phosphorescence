...
func getTracksFromPlaylist(playlistID string) (map[string]*TrackEnvelope, error) {
	nextURL := fmt.Sprintf("https://api.spotify.com/v1/playlists/%s/tracks?limit=100&fields=next,items(track(available_markets,id,name,popularity,album(id,images,name,artists),artists))", playlistID)
	...
	for nextURL != "" {
		...
		for _, item := range parsedBody.Items {
			...
			tracks[id] = &TrackEnvelope{Track: track}
		}
...
func getTrackFeatures(tracks map[string]*TrackEnvelope) (map[string]*TrackEnvelope, error) {
	...
	featuresByID := make(map[string]features)
	for _, audioFeatures := range parsedBody.AudioFeatures {
		id := audioFeatures.ID
		audioFeatures.ID = ""
		featuresByID[id] = audioFeatures
	}
	for id, track := range tracks {
		if track == nil {
			delete(tracks, id)
