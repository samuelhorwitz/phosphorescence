...
func getTracksFromPlaylist(playlistID string) (map[string]*TrackEnvelope, error) {
	nextURL := fmt.Sprintf("https://api.spotify.com/v1/playlists/%s/tracks?limit=100&fields=next,items(track(available_markets,duration_ms,external_urls,id,name,popularity,uri,album(external_urls,id,images,name,artists),artists))", playlistID)
